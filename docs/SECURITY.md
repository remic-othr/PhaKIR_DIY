...
### Data Protection
If working with sensitive medical data, keep all data storage (Garage, volumes, databases) secure. That might involve disk encryption at rest, though on a server in a secure facility this may be optional. The biggest risk is data leakage via the platform, which we mitigated by requiring logins for data access and by auditing who downloaded what (SFTP logs, etc.). Also, after the challenge, ensure that data access is revoked (e.g., disable those accounts or remove files from the download area).
### Web Application
